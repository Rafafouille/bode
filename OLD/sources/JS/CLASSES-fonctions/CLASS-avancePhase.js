...
var AvancePhase = function(paramKp,paramTi,paramA)
{
	...
	//==========================
		...
		//Redefinition
		this._type="Avance de Phase";//Nom de la fonction
		this._nom="Avance de Phase";//Nom de la fonction
		this._icone="./sources/icones/iconeAvancePhase.png";
		this._couleur="#BF8040";
	...
	//==========================
				...
				if(this.a()<1)
					{
						...
						if(w>1/(this.Ti()*this.a()))
							val+=-20*log10(1/this.a());//1/(this.Ti()*this.a()))+20*log10(1/this.Ti());
					}
				else
					{
						...
						if(w>1/this.Ti())
							val+=-20*log10(1/this.a());//1/this.Ti())-20*log10(1/(this.Ti()*this.a()));
					}
		...
		this.afficheEquation=function()
			{
				if(this.inverse())
					return '<span class="equation"><img src="http://latex.codecogs.com/gif.latex?\\tiny&space;H(p)=\\frac{1&plus;T_ip}{K_p(1&plus;aT_ip)}" title="\\tiny H(p)=\\frac{1+T_i}{K_p(1+aT_ip)}" /></span>';
				else
					return '<span class="equation"><img src="http://latex.codecogs.com/gif.latex?\\tiny&space;H(p)=K_p\\frac{1&plus;aT_ip}{1&plus;T_ip}" title="\\tiny H(p)=K_p\\frac{1+aT_ip}{1+T_i}" /></span>';
			}
...
'							<input type="range" name="'+i+'-input-param-a-range" id="'+i+'-input-param-a-range"  style="width:120px;" min="-5" max="5" step="0.1" value="'+log10(this.a())+'" oninput="updateParametreA('+i+',Math.pow(10,this.value))"/>'+
'						</div>';
				return res;
			}
		...
		/*this.ajouteLigneArbre_titre=function(i)
			{
				$("#arborescence").treetable('loadBranch',null,
					'<tr data-tt-id="'+i+'">'+
					'	<td>'+
					'		<img class="icone" src="./sources/icones/iconeAvancePhase.png"/>'+
					'		<strong>Cor.à avance de phase</strong> '+
		...
		this.fonctionBodeGdbAnalytiqueLaTex=function()
			{
				...
				if(this.inverse())
					signe="-";
				return signe+"\\APAmp{"+int2strfloat(this.Kp())+"}{"+int2strfloat(this.Ti())+"}{"+int2strfloat(this.a())+"}";
			}
		...
		this.fonctionBodeGdbAsymptotiqueLaTex=function()
			{
				...
				if(this.inverse())
					signe="-";
				return signe+"\\APAmpAsymp{"+int2strfloat(this.Kp())+"}{"+int2strfloat(this.Ti())+"}{"+int2strfloat(this.a())+"}";
			}
		...
		this.fonctionBodePhaseAnalytiqueLaTex=function()
			{
				...
				if(this.inverse())
					signe="-";
				return signe+"\\APArg{"+int2strfloat(this.Kp())+"}{"+int2strfloat(this.Ti())+"}{"+int2strfloat(this.a())+"}";
			}
		...
		this.fonctionBodePhaseAsymptotiqueLaTex=function()
			{
				...
				if(this.inverse())
					signe="-";
				return signe+"\\APArgAsymp{"+int2strfloat(this.Kp())+"}{"+int2strfloat(this.Ti())+"}{"+int2strfloat(this.a())+"}";
			}
...
AvancePhase.prototype.constructor = AvancePhase;//On recopie le constructeur de Noeud dans son prototype
